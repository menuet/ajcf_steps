...
public:
    SameAsTuple(int i, double d, std::string s) : m_i{i}, m_d{d}, m_s{s}
    {
    }
...
template <>
class std::tuple_size<SameAsTuple> : public std::integral_constant<std::size_t, 3>
{
};

template <std::size_t I>
class std::tuple_element<I, SameAsTuple>
{
public:
    using type = decltype(std::declval<SameAsTuple>().get<I>());
};

TEST_CASE("custom structured bindings", "[functional]")
{
    SameAsTuple same_as_tuple{1, 2.3, "nice"};

    auto [integer, real, text] = same_as_tuple;
